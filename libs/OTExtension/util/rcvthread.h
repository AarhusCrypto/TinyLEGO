...
public:
	...
	void flush_queue(uint16_t channelid) {
	...
	void remove_listener(uint16_t channelid) {
		rcvlock->Lock();
	...
	queue<uint8_t*>* add_listener(uint16_t channelid, CEvent* rcv_event, CEvent* fin_event) {
		rcvlock->Lock();
#ifdef DEBUG_RECEIVE_THREAD
...
//		assert(listeners[channelid].rcv_buf->empty());
	...
	void ThreadMain() {
		...
		while(true) {
			...
			rcv_len = 0;
			rcv_len += mysock->Receive(&channelid, sizeof(uint16_t));
